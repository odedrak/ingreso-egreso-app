...
export class IngresoEgresoService {
  ...
  initIngresoEgresoListener() {
    this.ingresoEgresoListenerSubscription = this.store.select('auth').pipe(
      // con el filtro la suscripcion solo es efectiva cuando el usuario no es nulo
  ...
  private ingresoEgresoItems(uid: String) {
    ...
    // Usamos el snapshotChanges para tener el uid de cada elemento
    this.ingresoEgresoItemSubscription = this.afDB.collection(`${uid}/ingresos-egresos/items`).snapshotChanges().pipe(
      map( docData => {
